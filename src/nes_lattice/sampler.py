...
def init_bundles(
    ...
    if move_type == "pair_flip":
        ...
        configs = jax.vmap(lambda kk: jax.random.permutation(kk, base))(keys)
    ...
    def sample(params, key, bundles):
        def step_with_params(carry, key):
            ...
            if move_type == "single_flip":
                ...
                raise ValueError(move_type)
            new_logabs = batch_logabsdet(apply_fun, params, proposal, det_jitter)
            logu = jnp.log(jax.random.uniform(key_u, (C,)) + 1e-12)
            accept = logu < 2.0 * (new_logabs - logabs)
        ...
        logabs0 = batch_logabsdet(apply_fun, params, bundles, det_jitter)
        keys = jax.random.split(key, burn_in)
        (bundles, logabs), acc_burn = jax.lax.scan(step_with_params, (bundles, logabs0), keys)

        def collect(carry, key):
            bundles, logabs = carry
            keys_inner = jax.random.split(key, sweep_steps)
            (bundles, logabs), acc = jax.lax.scan(step_with_params, (bundles, logabs), keys_inner)
            return (bundles, logabs), (bundles, jnp.mean(acc))

        keys_collect = jax.random.split(key, n_samples)
        (bundles, logabs), (samples, accs) = jax.lax.scan(collect, (bundles, logabs), keys_collect)
        flat_samples = samples.reshape((n_samples * n_chains, k, N))
        stats = {
            "accept_rate": jnp.mean(accs),
            "burn_accept_rate": jnp.mean(acc_burn) if burn_in > 0 else jnp.array(0.0),
        }
...
def make_config_sampler(
    ...
    def sample(params, key, configs):
        ...
        keys = jax.random.split(key, burn_in)
        (configs, logq), acc_burn = jax.lax.scan(step_with_params, (configs, logq0), keys)

        def collect(carry, key):
            configs, logq = carry
            keys_inner = jax.random.split(key, sweep_steps)
            (configs, logq), acc = jax.lax.scan(step_with_params, (configs, logq), keys_inner)
            return (configs, logq), (configs, jnp.mean(acc))

        keys_collect = jax.random.split(key, n_samples)
        (configs, logq), (samples, accs) = jax.lax.scan(collect, (configs, logq), keys_collect)
        flat_samples = samples.reshape((n_samples * n_chains, N))
        stats = {
            "accept_rate": jnp.mean(accs),
            "burn_accept_rate": jnp.mean(acc_burn) if burn_in > 0 else jnp.array(0.0),
        }
