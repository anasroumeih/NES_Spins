...
def nes_energy_matrix_variance(local_energy_mats) -> dict:
    """Estimate state energy variances from NES local energy matrices.

    Implements the practical form of Sec. S12 of the NES paper: estimate the
    mean local energy matrix, diagonalize it, transform every sampled local
    energy matrix to that eigenbasis, then take the variance of the diagonal
    elements.  This is equivalent to transforming the covariance of the
    vectorized local energy matrix.
...
def variance_from_bundles(apply_fun, params, bundles, hspec, bonds) -> dict:
    """Compute NES S12 variance statistics on a batch of determinant bundles."""
    mats = batch_local_energy_matrix(apply_fun, params, bundles, hspec, bonds)
    return nes_energy_matrix_variance(np.asarray(jax.device_get(mats)))
